...
基础依赖见 `requirements.txt`。规划和评估默认使用 MOSEK 原生 Task API；`milp_solver.py` 仍保留 Gurobi、CPLEX、MOSEK、SciPy 的显式选择和自动回退能力，便于测试或特殊环境手工切换。
...
- 上限场景自动值：`-负荷 * 负荷向下扰动系数`
- `是否考虑负荷扰动=0` 时，负荷向上/向下扰动系数按 0 处理；`是否考虑新能源扰动=0` 时，新能源向下扰动系数按 0 处理。
- 网页不再提供手动覆盖值；频率扰动功率完全由“扰动后安全参数”中的扰动系数自动计算。
- 结果工作簿和曲线列表中仍会输出自动计算得到的频率下限/上限扰动功率，便于复核 Delta P。

旧字段 `频率安全上限(1.0-1.5)`、`频率安全下限(0.5-1.0)`、`频率下限扰动功率(kW)` 和 `频率上限扰动功率(kW)` 已从网页、工作簿字段定义和校验逻辑中删除。
...
- 频率安全约束是否过严，例如 Nadir/Peak 限值、RoCoF 上限、扰动系数。
- 储能 SOC 上下限和初始 SOC 是否与运行需求冲突。
- 绿色电量占比下限是否过高。

### 没有明显频率波动曲线

频率扰动功率已完全由“扰动后安全参数”自动计算。若 `是否考虑负荷扰动=0`、`是否考虑新能源扰动=0`，或对应扰动系数均为 0，则频率曲线会保持在额定频率附近。需要看到频率波动时，应开启相应扰动开关并设置非零扰动系数。
