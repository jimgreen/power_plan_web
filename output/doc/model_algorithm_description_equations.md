...
系统调度时间尺度为小时级，全年共 8760 个点。模型在每个小时同时考虑负荷平衡、柴油发电机出力、风电和光伏最大可发、弃电、储能充放电、制氢、储氢、燃料电池发电、切负荷以及可选的扰动后平衡约束。输出结果包括小时级曲线、日级统计、月度统计和年度指标。
...
其中，$x$ 表示连续运行变量，$y$ 表示整数台数变量，$z$ 表示充放电互斥等二进制状态变量。模型的主要工程目标是，在满足设备数量边界、设备运行约束、安全约束和绿电占比约束的前提下，使年均建设成本、柴油成本和切负荷惩罚最小。
...
规划求解目标函数由年均建设成本、柴油运行成本和切负荷惩罚组成。设 $c_i^{\mathcal{D}}$ 为第 $i$ 行设备单台建设成本，单位为万元/台；$Y_i^{\mathcal{D}}$ 为设计年限，单位为年。设备年均建设成本为：
...
其中 $\pi_{\mathrm{diesel}}$ 为柴油价格，单位为万元/吨。切负荷量采用大权重惩罚，促使模型只有在确实无法供电时才切负荷：
...
每个小时必须满足电力平衡。供电侧包括柴油发电、风电、光伏、储能放电和燃料电池发电；用电侧包括负荷、储能充电、电制氢和切负荷修正。
...
由于切负荷有大惩罚，正常可行情况下该变量应接近 0。若结果中出现较大的切负荷功率，应优先检查柴油、风光、储能和燃料电池容量是否不足，以及扰动后平衡约束是否过紧。
